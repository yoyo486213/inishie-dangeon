...
UpStairs::UpStairs(const std::string& ImagePath, Util::Renderer *m_Root) : Character(ImagePath) {
    std::vector<std::string> TextButtonImages;
    TextButtonImages.reserve(2);
    for (int i = 1; i < 3; i++) {
        char buffer[200];
        snprintf(buffer, sizeof(buffer), RESOURCE_DIR"/Map/UpStairsText/UpStairsText-%d.png", i);
...
bool UpStairs::IsCollision(const std::shared_ptr<Character> &other, glm::vec2 displacement) {
