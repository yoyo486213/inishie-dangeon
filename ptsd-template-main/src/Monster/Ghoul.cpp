...
Ghoul::Ghoul() : Monster({RESOURCE_DIR"/Monster/Ghoul/Ghoul-B-0.png", RESOURCE_DIR"/Monster/Ghoul/Ghoul-B-1.png",
                          RESOURCE_DIR"/Monster/Ghoul/Ghoul-F-0.png", RESOURCE_DIR"/Monster/Ghoul/Ghoul-F-1.png",
                          RESOURCE_DIR"/Monster/Ghoul/Ghoul-L-0.png", RESOURCE_DIR"/Monster/Ghoul/Ghoul-L-1.png",
                          RESOURCE_DIR"/Monster/Ghoul/Ghoul-R-0.png", RESOURCE_DIR"/Monster/Ghoul/Ghoul-R-1.png"}, 
                        250, 0, glm::vec2{22, 32}, 9, 105, 5, std::vector<int>{0, 0, 0, 0, 0}, 125, 500, 56.0) {
    this->goalpos = this->GetPosition();
}

void Ghoul::Move(glm::vec2 displacement, glm::vec2 goal, std::vector<std::shared_ptr<Monster>> m_Monsters) {
...
void Ghoul::Update(std::shared_ptr<Player> &m_Player, std::vector<std::shared_ptr<Character>> AllObjects, std::vector<std::shared_ptr<ICollidable>> AllCollidableObjects, std::vector<std::shared_ptr<InvisibleWall>> m_Invisiblewalls, std::vector<std::shared_ptr<Monster>> m_Monsters) {
    // 初始化隨機數生成器
    ...
    if (this->GetHP() > 0 && m_Player->GetHP() > 0 && (this->state == State::Stop || static_cast<int>(glm::distance(pos, goalpos)) <= 45)) {
        // 往旁邊走攻擊距離看有沒有碰到玩家
    ...
    if (this->state == State::Stop) {
        ...
        if (glm::distance(m_Player->GetPosition(), this->GetPosition()) <= 140) {
            this->following = true;
        }
        if (following) {
            int mindis = 99999;
